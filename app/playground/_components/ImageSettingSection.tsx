...
function ImageSettingSection({ selectedEl }: Props) {
  ...
  const generateAiImage = async () => {
    ...
    const promptText = encodeURIComponent(altText || "A professional portrait");
    const uniqueSuffix = Date.now();
    const imageUrl = `https://ik.imagekit.io/dtgpthykf/ik-genimg-prompt-${promptText}/gen-${uniqueSuffix}.jpg?tr=`;
  ...
  return (
    <div className="w-96 shadow p-4 space-y-4">
      ...
      <Button
        ...
        disabled={isLoading}
      >
        {isLoading && <Loader2Icon className="animate-spin" />}Upload Image
      </Button>
